...
export const rewardsApi = {
  ...
  getPointsHistory: async (page: number = 1, limit: number = 20) => {
    return apiCall<{ transactions: PointsTransaction[]; total: number; page: number; pages: number }>(
  ...
  getRewards: async (filters?: {
    ...
    return apiCall<Reward[]>(
      apiClient.get(`/rewards?${params.toString()}`)
    );
  },
  ...
  getRewardById: async (rewardId: string) => {
    return apiCall<Reward>(
      apiClient.get(`/rewards/${rewardId}`)
    );
  },
  ...
  redeemReward: async (rewardId: string) => {
    return apiCall<RewardRedemption>(
      ...
      { showLoading: true, showSuccess: true, successMessage: 'Reward redeemed successfully!' }
    );
  },
  ...
  getRedemptions: async (filters?: {
    ...
    return apiCall<{ redemptions: RewardRedemption[]; total: number; page: number; pages: number }>(
  ...
  getStats: async () => {
    return apiCall<RewardsStats>(
      apiClient.get('/rewards/stats')
    );
  },

  // Get leaderboard
  getLeaderboard: async (period: 'week' | 'month' | 'all' = 'all', limit: number = 50) => {
    return apiCall<{
      leaderboard: {
        userId: string;
        username: string;
  ...
  checkRewardAvailability: async (rewardId: string) => {
    return apiCall<{ available: boolean; stock?: number; canAfford: boolean; pointsNeeded: number }>(
  ...
  claimDailyBonus: async () => {
    return apiCall<{
      ...
      { showSuccess: true, successMessage: 'Daily bonus claimed!' }
    );
  },
  ...
  getCategories: async () => {
    return apiCall<{ category: string; count: number; label: string; icon: string }[]>(
